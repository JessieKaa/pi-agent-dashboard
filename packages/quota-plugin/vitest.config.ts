...
export default defineConfig({
  ...
  resolve: {
    ...
    // test — `ComposerContextGroupSlot`/`ComposerContextGroup` exist only in
    // this worktree until the change lands. Mirrors packages/client +
    // packages/blackhole-plugin. Specific subpath keys MUST precede the bare
    // key (alias matches by prefix). See change: move-quota-to-context-strip.
    alias: {
