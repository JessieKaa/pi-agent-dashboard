...
| `memory-event-store.ts` | In-memory event store with LRU eviction; replaces SQLite-backed event-store. → see `memory-event-store.ts.AGENTS.md`. Exports `truncateStrings` (reused by the transcript→diff projection so payloads match). See change: fix-session-diff-durable-source. |
