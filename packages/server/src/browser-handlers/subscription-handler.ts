...
export async function sendEventBatches(
  ...
  windowLimitOrMetadata?: number | HistoryWindowMetadata,
  mode: ReplayWindowMode = "head-tail",
): Promise<number> {
  const legacyWindow = windowLimitOrMetadata && typeof windowLimitOrMetadata === "object"
    ? windowLimitOrMetadata
    : undefined;
  const windowLimit = typeof windowLimitOrMetadata === "number" ? windowLimitOrMetadata : undefined;
  const preCompactionMaxSeq = stored.length > 0 ? stored[stored.length - 1].seq : legacyWindow?.endSeq ?? 0;
  const full = compactEventsForReplay(stored);
  const replayWindow = legacyWindow ? null : computeReplayWindow(full, windowLimit ?? 0, mode);
  let compacted = legacyWindow ? compactReplayEvents(stored) : full;
  if (legacyWindow) {
    if (compacted.length === 0) {
      if (ws.readyState !== ws.OPEN) return 0;
      sendTo(ws, { type: "event_replay", sessionId, events: [], isLast: true, historyWindow: legacyWindow });
      return preCompactionMaxSeq;
    }
  } else if (replayWindow) {
    if (ws.readyState === ws.OPEN) sendTo(ws, { type: "session_state_reset", sessionId });
  ...
  if (compacted.length === 0) {
    if (ws.readyState !== ws.OPEN) return 0;
    sendTo(ws, { type: "event_replay", sessionId, events: [], isLast: true, ...(legacyWindow ? { historyWindow: legacyWindow } : {}) });
    return preCompactionMaxSeq;
  }
  for (let i = 0; i < compacted.length; i += REPLAY_BATCH_SIZE) {
    ...
    sendTo(ws, {
      ...
      ...(legacyWindow && i + REPLAY_BATCH_SIZE >= compacted.length ? { historyWindow: legacyWindow } : {}),
    });
    if (ws.bufferedAmount > BACKPRESSURE_THRESHOLD) {
      await new Promise<void>((resolve) => {
        const check = () => {
          if (ws.readyState !== ws.OPEN || ws.bufferedAmount < BACKPRESSURE_THRESHOLD) resolve();
...
export function handleSubscribe(
  ...
  if (eventStore.hasEvents(msg.sessionId) && !canForceRefreshFromDisk) {
    ...
    if (selection) {
      ...
      sendEventBatches(ws, msg.sessionId, events, sendTo, selection.metadata)
        .then((lastSent) => {
      ...
      directoryService.loadSessionEvents(msg.sessionId, session.sessionFile, session.contextWindow).then(async (result) => {
        ...
        if (result.success) {
          ...
          if (forceRefresh) {
            ...
            for (const sub of getSubscribers(msg.sessionId)) {
              if (sub.readyState === sub.OPEN) sendTo(sub, { type: "session_state_reset", sessionId: msg.sessionId });
            }
            ...
            for (const evt of liveEvents) eventStore.insertEvent(msg.sessionId, evt.event);
          } else {
            for (const evt of result.events) {
              const prepared = prepareEventForIngest(evt);
              pendingAttachments.push(...prepared.pending);
          ...
          for (const sub of subscribers) {
            ...
            const selection = preference ? selectReplayWindow(stored, preference.historyWindow.messages) : null;
            if (selection && preference) {
              const boundaryMatches = preference.historyWindow.firstSeq != null && preference.historyWindow.firstSeq === selection.metadata.startSeq;
              const cursorFits = preference.lastSeq <= selection.metadata.endSeq;
              if (!boundaryMatches || !cursorFits) sendTo(sub, { type: "session_state_reset", sessionId: msg.sessionId });
            }
            markReplaying(sub, msg.sessionId);
            const lastSent = await sendEventBatches(
              sub, msg.sessionId, selection?.events ?? stored, sendTo,
              selection?.metadata ?? (selection ? undefined : maxReplayEvents),
              selection ? undefined : replayWindowMode,
            );
