...
/**
 * The staleness-hash plugin set: EXACTLY what the server hashes at runtime
 * (`discoverPlugins()` minus `fixture: true` in production) — deliberately
 * broader than `loadPluginEntries`, which additionally requires a client
 * entry because it feeds import generation. Server-only plugins (e.g.
 * `mcp-server`, no `client` field) are in the runtime set, so they must be
 * in the embedded and declared hashes too or the staleness banner shows
 * forever. Do NOT couple this to clientEntryPath or any filesystem-topology
 * signal: the server runs against a different install (global npm) and only
 * manifests are guaranteed comparable.
 * See change: optimize-client-bootstrap-and-bundle-coherence (P0 D3).
 */
function registryHashForRoot(repoRoot: string, isProd: boolean): string {
  clearDiscoveryCache();
  const plugins = discoverPlugins(repoRoot).filter(
    p => !(isProd && p.manifest.fixture === true),
  );
  return pluginRegistryHash(plugins);
}
 ...
 */
function generateRegistryContent(entries: PluginEntry[], repoRoot: string, isProd: boolean): string {
  ...
  // Computed over the runtime staleness set (`registryHashForRoot`), NOT
  // over `entries` — see that helper for why. See change: fix-pi-flows-end-to-end
  // (Group 6).
  const hash = registryHashForRoot(repoRoot, isProd);
...
function regenerate(repoRoot: string, isProd: boolean): { changed: boolean; content: string } {
  ...
  const content = generateRegistryContent(entries, repoRoot, isProd);
...
export function viteDashboardPluginsPlugin(repoRoot?: string): Plugin {
  ...
  return {
    ...
    closeBundle() {
      ...
      writeBuildDeclaration(buildOutDir, registryHashForRoot(root, true));
