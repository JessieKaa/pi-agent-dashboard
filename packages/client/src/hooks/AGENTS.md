...
| `useArchivedSessions.ts` | Per-key lazy cache for archived-session listings (`<groupPath>` for folder folds, `q:<text>` for search). → see `useArchivedSessions.ts.AGENTS.md` |
...
| `useBodyDragStyle.ts` | NEW. `useBodyDragStyle(): { beginBodyDrag(cursor), endBodyDrag() }` — sets/clears… → see `useBodyDragStyle.ts.AGENTS.md` |
...
| `useIdleFx.ts` | `useIdleFx()` — 5000 ms (`IDLE_FX_DELAY_MS`) input-idle detector; toggles `fx-idle` on… → see `useIdleFx.ts.AGENTS.md` |
...
| `useMessageHandler.replay-coalescing.test.tsx` | Regression coverage for replay frame coalescing, reset/continuation ordering, cursor and persistence side… → see `useMessageHandler.replay-coalescing.test.tsx.AGENTS.md` |
| `useMessageHandler.ts` | Dispatches `ServerToBrowserMessage` events to client state: cursor/plugin/cache/loading side effects, coalesced replay/live ordering, pending-prompt settlement, replay metadata, sessions_page_result/openspec/snapshot/endedTotals/archived arms. → see `useMessageHandler.ts.AGENTS.md` |
...
| `useOpenSpecReconcile.ts` | New. Reconciliation pull (D7): sends one `openspec_get` per rendered cwd with no settled OpenSpec entry and… → see `useOpenSpecReconcile.ts.AGENTS.md` |
...
| `useSessionActions.ts` | Session action callbacks extracted from App.tsx. Sends… → see `useSessionActions.ts.AGENTS.md` Swaps… → see `useSessionActions.ts.AGENTS.md` |
