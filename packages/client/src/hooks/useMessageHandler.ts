...
export function useMessageHandler(
  ...
  const clearHistoryWindow = useCallback((sessionId: string) => {
    ...
    setSessionStates((prev) => {
      ...
      for (const [sessionId, batches] of drained) {
        ...
        for (const batch of batches) {
          ...
          const gap = historyGapsRef.current.get(sessionId);
  ...
  return useCallback((msg: ServerToBrowserMessage) => {
    ...
    switch (msg.type) {
      ...
      case "session_state_reset":
        ...
        clearHistoryWindow(msg.sessionId);
      ...
      case "event_replay": {
        ...
        // NON-terminal batch is a liveness signal that re-arms the ceiling.
        // See change: show-replay-in-flight-indicator.
        if (msg.isLast === true) {
          clearLoadingHistory(setReplayInFlight, replayInFlightTimersRef, msg.sessionId);
          // ARM backfill only now: before the terminal batch an evicted cold
          // session's store is still empty and an early request can resolve the
          // gap before hydration lands.
