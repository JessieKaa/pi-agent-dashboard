...
function mount(): { get: () => Harness; fire: (m: ServerToBrowserMessage) => void } {
  ...
  // Gap tests assert replay output, not frame batching. Drain the scheduled
  // replay publication in a second act; replay-coalescing tests own timing.
  const fire = (message: ServerToBrowserMessage) => {
    act(() => latest.handle(message));
    if (message.type !== "event_replay" || message.events.length === 0) return;
    act(() => {
