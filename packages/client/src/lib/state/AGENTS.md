...
| `draft-store.ts` | Keyed external draft store. `useSessionDraft` subscribes only one Session via `useSyncExternalStore`; updates persist after 300ms and `clearDraft` deletes eagerly. |
