...
| `scroll-debug.ts` | Gated console logging for scroll-landing diagnosis. `SCROLL_DEBUG_KEY` (`pi-debug-scroll`, localStorage `"1"` enables), `scrollDebugEnabled()`, `scrollDebugLog(event, data?)` — zero-cost no-op when off; entries `[scroll-debug] <event> {t, …}`. See change: fix-ux-degradation-long-session. |
