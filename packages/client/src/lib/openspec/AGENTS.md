...
| `openspec-config-api.ts` | Fetch helpers for OpenSpec config + update endpoints. Adds saveOpenSpecConfig(), runOpenSpecUpdate(), fetchUpdateStatus() + OpenSpecUpdateStatus types. See change: add-openspec-profile-settings. Adds `runOpenSpecInit` (+`OpenSpecInitError` w/ stderr), `fetchOpenSpecPollSettings`, `setOfferInitialization`, `addOpenSpecOptOut`/`removeOpenSpecOptOut` (GET→PUT read-modify-write on /api/config). See change: add-openspec-init-affordances. `useOpenSpecConfig` shares ONE in-flight config request per cwd across concurrent hooks (module-level map; a per-hook `cancelled` flag replaces the AbortController — an unmount must not abort a sibling's shared request) + 30 s failure negative-cache (`OPENSPEC_CONFIG_FAILURE_TTL_MS`); success or `__resetOpenSpecConfigCache` (save/epoch path) clears the failure entry; seed-then-refetch semantics unchanged. See change: trim-cold-start-transfer-and-config-fanout (④). |
