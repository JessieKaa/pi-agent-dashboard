...
| `folder-worktree-availability.ts` | Single folder-level "can a worktree be spawned here?" rule. Exports `resolveWorktreeAvailability({cwd,sessions,folderGitMap,gitWorktreeEnabled,platform?})` → `{available, reason?: "not-a-git-repo"\|"worktrees-disabled"}`, plus `WorktreeAvailability`. Preference gate first; `folderGitMap` is POSITIVE evidence only (`null` = unknown); then session `isGitRepo === false`; else fail-open. Never reads `gitBranch` (liveness-coupled). Folder/session matching via shared `pathKey`. See change: fix-openspec-board-worktree-button-gating. |
...
| `init-status-cache.ts` | Module-level resolved-cwd cache + in-flight dedup for the worktree init-status probe (`useInitStatus`, A5). `getCachedInitStatus(cwd)` / `setCachedInitStatus(cwd, status)` / `getInflightInitStatus(cwd)` / `setInflightInitStatus(cwd, p)` / `clearInflightInitStatus(cwd)`; failures are NOT cached. See change: fix-archive-feedback-and-sidebar-perf. |
