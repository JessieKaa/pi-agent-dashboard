...
const CENSUS: Record<string, string[]> = {
  ...
  "__tests__/chat-input-draft-integration.test.tsx": [
    // Renamed by the draft-store rework (perf(client) reduce-long-session-render-latency)
    // — the file asserts the SessionCommandInput + draft-store architecture now;
    // this census tracks the merged tree.
    "draft survives unmount/remount of the chat view",
    "drafts do not leak between sessions",
    "hydrates drafts from localStorage",
    "persists drafts after the debounce",
    "clear updates a mounted composer and removes storage",
    "typing does not rerender the parent shell",
  ],
