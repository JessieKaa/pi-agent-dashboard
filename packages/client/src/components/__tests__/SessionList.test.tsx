...
describe("SessionList compact workspace sidebar", () => {
  ...
  it("keeps the folder header, Git info, project actions, body, and session card", () => {
    ...
    fireEvent.click(screen.getByTestId("folder-actions-menu-/my/project"));
    expect(screen.getByRole("menuitem", { name: "Directory Settings" })).toBeTruthy();
  ...
  it("hides dashboard-level folder and workspace creation controls", () => {
    render(
      <TestRouter>
        <ThemeProvider>
          <SessionList
    ...
    expect(screen.queryByTestId("dashboard-add-folder-btn")).toBeNull();
    expect(screen.queryByTestId("dashboard-new-workspace-btn")).toBeNull();
  });

  it("keeps the folder name untruncated in compact mode", () => {
    const { container } = renderCompact(true);
    const folderName = container.querySelector('[data-testid="folder-header-leaf-/my/project"]');
