...
| `ChatView.tsx` | `msg.view` rows render as `<PreviewCard target={msg.view}>` (right-aligned, `bubbleMax` width) BEFORE… → see… → see `ChatView.tsx.AGENTS.md` |
| `ChatViewMenu.tsx` | Discord-style ⚙ View popover mounted in chat toolbar. Edits per-session `displayPrefsOverride` via… → see… → see `ChatViewMenu.tsx.AGENTS.md` |
| `collapse-summary.tsx` | Shared PROCESS-subcard summary primitives: `splitOverflow<T>` (pure head/overflow-tail sort-then-slice split)… → see `collapse-summary.tsx.AGENTS.md` |
...
| `CommandInput.tsx` | Chat composer with controlled/uncontrolled draft + image state, paste/upload previews, provider/model refresh errors, model/thinking controls, slash/@ autocomplete (local `/view`), terminal control, steer/queue delivery, history recall, pending-prompt status handling. → see `CommandInput.tsx.AGENTS.md` |
| `HistoryGapDivider.tsx` | Click-to-load interstitial disclosing a windowed replay's elided middle. → see `HistoryGapDivider.tsx.AGENTS.md` |
| `MultiAskPanel.tsx` | Grouped multi-ask panel (`data-testid="multi-ask-panel"`) for concurrently-pending free-floating asks. → see `MultiAskPanel.tsx.AGENTS.md` |
...
| `CustomEntryCard.tsx` | Bounded generic fallback card for non-`flow-event` custom content (pi.sendMessage custom messages +… → see `CustomEntryCard.tsx.AGENTS.md` |
