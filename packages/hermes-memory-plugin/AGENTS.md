...
| `vitest.config.ts` | Vitest config. `@vitejs/plugin-react`, jsdom, `pool: forks`, `maxWorkers: PARALLEL_MAX_WORKERS` (repo-root `vitest.workers.ts`), globalSetup `@blackbelt-technology/pi-dashboard-shared/test-support/setup-home.ts`. |
