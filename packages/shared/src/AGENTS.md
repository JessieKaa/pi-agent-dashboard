...
| `__tests__/knip-workflow-contract.test.ts` | CI placement contract for the dead-code oracle (test-plan #X1-#X3). → see `__tests__/knip-workflow-contract.test.ts.AGENTS.md` |
...
| `__tests__/site-deploy-workflow-contract.test.ts` | Static contracts for change fix-deploy-site-ship-shell (E9–E15): deploy-site.yml push trigger stays… → see `__tests__/site-deploy-workflow-contract.test.ts.AGENTS.md` |
...
| `__tests__/state-replay.test.ts` | L1 for the persisted `compaction` → `session_compact` replay arm (change: replay-compaction-boundary, E1–E7 + X1): boundary index/timestamp between neighbours, no-compaction golden sequence, 0/1/2/3 multiplicity, first/last position, no fabricated `reason`/`willRetry`/`estimatedPostCompactionTokens`, 64 KB `summary` never emitted, schema drift, malformed entry does not abort. || `__tests__/config-keeper-log.test.ts` | keeperLog parse boundaries (test-plan #E7): valid values pass; 0/−1/non-numeric/absent coerce to defaults; `capturePiOutput` unaffected. See change: fix-runaway-keeper-log-growth. |
| `__tests__/transcript-request-guard.test.ts` | L1 for the guard (#E13, #E14): own-session allow, foreign-session refusal, every path-bearing field refused on FIELD presence (not value shape), path-before-subject ordering, static no-fs-import proof, refuse-before-`session_register`. Moved from `extension/src/__tests__`. See change: serve-retained-remote-transcripts. || `zrok-env.ts` | Pure `readZrokEnvironment({homedir,fs})` returning `{found, kind: v2\|v1\|null, path, env, reason}`. → see `zrok-env.ts.AGENTS.md` |
| `archive-types.ts` | `ArchiveEntry` interface — OpenSpec archive dir row (name, date, done artifacts). Shared server↔client. |
| `bind-reachability.ts` | Bind-vs-trust reachability: the ONE predicate + well-known-range table, imported by both client and server so… → see `bind-reachability.ts.AGENTS.md` |
...
| `browser-protocol.ts` | Server↔Browser WebSocket contracts. Exports message unions plus `HistoryWindowMetadata`;… → see `browser-protocol.ts.AGENTS.md` |
...
| `display-prefs.ts` | `DisplayPrefs` interface gates chat-view chrome (thinking, tool calls per kind, turn separators, debug… → see `display-prefs.ts.AGENTS.md` |
...
| `grammar-types.ts` | Wire contract for composer grammar/spell check (client↔server). → see `grammar-types.ts.AGENTS.md` |
...
| `host-pressure.ts` | Host-pressure thresholds, ONE source of truth. Exports `HOST_PRESSURE_DEGRADED_MS` (35_000), `HOST_PRESSURE_UNRESPONSIVE_MS` (60_000). Server `host-pressure-tracker.ts` FIRES on them; `SessionCard.tsx` escalates degraded → unresponsive BETWEEN transitions on them. Both re-export, neither redefines — a private copy drifts and the card escalates ahead of or behind the server. See change: fix-false-unresponsive-badge. || `index.ts` | Barrel re-exports for cumbersome symbols. Re-exports `doctor-core.js`, `node-version.js`, `ViewTarget` type,… → see `index.ts.AGENTS.md` |
...
| `instance-directory.ts` | `listLocalInstances()` / `resolveInstanceRef()` / `formatInstanceLine()` — enumerates `gateway-*.sock` under… → see `instance-directory.ts.AGENTS.md` |
...
| `mdns-discovery.ts` | mDNS `_pi-dashboard._tcp` advertise + discover. `advertiseDashboard(port, piPort, {bindHost?, publish?})`… → see `mdns-discovery.ts.AGENTS.md` |
| `memory-limits.ts` | BROWSER-SAFE memory-limit types + defaults: `MemoryLimitsConfig`, `MIN_REPLAY_WINDOW` (100),… → see `memory-limits.ts.AGENTS.md` |
...
| `openspec-poller.ts` | Aggregates `openspec list`+`status` into `OpenSpecData`. `pollOpenSpec(cwd)` sync (bridge),… → see… → see `openspec-poller.ts.AGENTS.md` |
...
| `path-containment.ts` | Boundary-correct `isPathInside(parent, child, platform?)` (sibling prefix `/repo-other` is NOT inside… → see `path-containment.ts.AGENTS.md` |
...
| `rest-api.ts` | REST endpoint request/response types. Sessions, events, spawn, file read/write (`FileWriteRequest` mtime… → see `rest-api.ts.AGENTS.md` || `role-name-validation.ts` | Shared role-name trust boundary. `isValidRoleName(name, existing) → {ok, reason?}`: non-empty after trim;… → see `role-name-validation.ts.AGENTS.md` |
...
| `role-schema.ts` | Pure, `node:fs`-free role-schema core shared by every role read surface (bridge `role-manager.ts`,… → see `role-schema.ts.AGENTS.md` |
| `route-tiers.ts` | `ROUTE_TIERS`, `routeTier(method, pattern)`, `hasRouteTier`. ONE route→tier map for `/api/*`; unlisted → `operate` (fail closed). Read by the REST tier gate AND the MCP manifest's `rest` rows, so both agree. See change: expand-mcp-tiered-surface. |
...
| `server-launcher.ts` | `launchDashboardServer` — single shared spawn primitive (jiti loader, argv, env, log header, readiness) used… → see `server-launcher.ts.AGENTS.md` |
| `server-pinning.ts` | Client-side D8 pinning of the server Ed25519 identity. `fingerprintFromPublicKeyB64`, `verifyNonceSignature`,… → see `server-pinning.ts.AGENTS.md` |
...
| `session-meta.ts` | Per-session sidecar `.meta.json` schema + IO. Exports `SessionMeta`, `metaPath`, `readSessionMeta`,… → see `session-meta.ts.AGENTS.md` || `stats-extractor.ts` | Extracts `StatsData` (tokensIn/out, cost, turnUsage, optional contextUsage) from a `turn_end` event's… → see `stats-extractor.ts.AGENTS.md` |
...
| `terminal-types.ts` | Terminal emulator shared types. Exports `TerminalSession` (id, cwd, shell, status, title, `ephemeral` flag for inline cards) and `TerminalControlMessage` (resize | title) control frame union. → see `terminal-types.ts.AGENTS.md` |
...
| `tunnel-concurrency.ts` | Resolves WHICH providers run and in which mode. `resolveTunnelPlan`/`primaryOf`/`soleMode`. → see `tunnel-concurrency.ts.AGENTS.md` |
