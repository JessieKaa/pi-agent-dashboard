...
| `vitest.config.ts` | Vitest project config. jsdom env, forks pool, `@vitejs/plugin-react`, `PARALLEL_MAX_WORKERS`. `resolve.alias` maps `@blackbelt-technology/pi-dashboard-shared` → `../shared/src` so tests see WORKTREE-LOCAL shared source; the hoisted-workspace symlink escapes to the main checkout and would hide a new shared export. Mirrors server/client/extension configs. See change: add-git-checkout-root-resolver. |
