...
| `package.json` | Package manifest. Name `@blackbelt-technology/pi-dashboard-nano-banana`. `pi.skills` → `.pi/skills/nano-banana-imagegen`, `pi.tools` probes `GEMINI_API_KEY`. bin `pi-nano-banana` → `src/bin/nano-banana.ts`. `exports` map `./*.js`→`./src/*.ts` for cross-package TS import. dep `@blackbelt-technology/pi-dashboard-shared`. |
