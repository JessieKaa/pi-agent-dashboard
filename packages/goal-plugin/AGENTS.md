...
| `vitest.config.ts` | Vitest config for goal-plugin. `defineConfig` with `@vitejs/plugin-react`. Includes `src/**/__tests__/**/*.test.{ts,tsx}`, jsdom env, `pool: "forks"`, `maxWorkers: PARALLEL_MAX_WORKERS`, globalSetup `@blackbelt-technology/pi-dashboard-shared/test-support/setup-home.ts`, setupFiles `setup-home-perfile.ts`. |
