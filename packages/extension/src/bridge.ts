...
import { consumeSessionReplacementHandoff, extractFirstMessage, extractLatestTurnWindow, filterHiddenCommands, getCurrentModelString, isHeadlessRpcSession, safeCwd } from "./bridge-context.js";
...
import { COALESCE_WINDOW_MS, type MessageKey, MessageUpdateCoalescer } from "./message-update-coalescer.js";
...
interface BridgeState {
  ...
  /** One-shot ownership grant for pi session replacement commands. */
...
function initBridge(pi: ExtensionAPI) {
  ...
  // Window sweep for the session-scoped message_update coalescer. Each slot
  // tracks its own arm time; the shared interval cannot delay a window.
  ...
  // Generation barrier prevents late message_update snapshots crossing message
  // or session boundaries. Every message_start advances it, including user turns.
  let assistantMessageGen = 0;
  // Session-scoped coalescer for contiguous text snapshots. Non-text updates
  // flush synchronously so source order remains intact.
  ...
  let connection = new ConnectionManager({
    ...
    onReconnect: safe(() => {
      if (!isActive()) return; // Stale listener guard
      // ConnectionManager drains its buffered message_start frames after this
      // callback returns. Flush parked updates in the next microtask so their
      // opening message is always on the wire first.
      queueMicrotask(safe(() => {
        if (isActive()) updateCoalescer.flush();
      }));
  ...
  for (const eventType of enrichedEventTypes) {
    pi.on(eventType as any, safe(async (event: any, ctx: any) => {
      ...
      // Flush parked text before every non-update event. This keeps a final
      // snapshot ahead of message_end and other lifecycle boundaries.
      if (eventType !== "message_update") updateCoalescer.flush();
      ...
      // Coalesce contiguous text snapshots; thinking/toolcall/unknown updates
      // flush preceding text and forward immediately to preserve source order.
  ...
  function sendEventForward(channel: string, data: Record<string, unknown>): void {
    // EventBus frames are ordering boundaries for parked text snapshots.
    updateCoalescer.flush();
  ...
  function handleSessionChange(ctx: any) {
    ...
    // inherit a straggler from the outgoing one.
