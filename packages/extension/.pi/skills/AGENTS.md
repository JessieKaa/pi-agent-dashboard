...
| `project-init/profiles/coding/AGENTS.md.tmpl` | Coding-profile root `AGENTS.md` template. Overview + Code Instructions (think/simplicity/surgical/TDD/communication) + stack-substituted Commands + OpenSpec + Discipline-Skills table. Carries NO "Finding docs (READ discipline)" section — kb-extension injects READ+WRITE doctrine per turn; template copy would double-load and drift. Invariant asserted by `src/__tests__/project-init-doctrine-seeds.test.ts`. |
