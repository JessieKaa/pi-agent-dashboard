...
- **`folderGitMap: Map<string, string | null>`** — client state (`App.tsx:558`) fed by `git_head_update` broadcasts from the folder-HEAD poll; replayed per-socket on connect (`browser-gateway.ts:598`). Keys are the DISPLAY paths the poll broadcasts (`git_head_update.cwd`, stored raw at `useMessageHandler.ts:737`) — NOT pre-folded, so a consumer must normalize on lookup rather than `map.get(pathKey(cwd))`. Covers folders with **no** sessions (pinned dirs). Currently passed only to `SessionList` (`App.tsx:1590`).
