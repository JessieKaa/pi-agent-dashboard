...
- [Exact lag vs the pinned at-rest format] → The pinned extension (pi-blackhole@0.4.10) records hash `entryId` cursors; numeric `entry`/`tip` positions (the spec's E11 model) are accepted by the reader but null at rest today. So the exact lag readout renders only when numeric positions are recorded (fixtures, or a future extension that persists them), and a rendered proximity meter can momentarily lack its exact companion on real data. Accepted: inventing a numeric proxy for hash cursors would fabricate the very false precision the spec forbids elsewhere. See change: add-blackhole-session-pipeline (review round 1, finding F2).
