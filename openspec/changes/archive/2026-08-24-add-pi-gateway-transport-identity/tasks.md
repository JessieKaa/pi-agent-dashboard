...
- [x] 12.52 **Deferred here; DELIVERED by `serve-retained-remote-transcripts`** as `tests/e2e/remote-transcript-read.spec.ts` ("history predating the attach is served, in order, and renders"), proven load-bearing by a revert run. New spec modelled on `tests/e2e/large-session-replay.spec.ts` — pre-attach history renders. Triple: remote session with entries predating the bridge attach · open the transcript · pre-attach entries present in the rendered transcript. (test-plan #F6)
  - **Why it was deferred (historical):** at the time this change shipped, `RemoteTranscriptStore` was written by the `transcript_chunk` handler and its `read()` had NO caller, no HTTP route, and no client surface. There was no rendered transcript for a browser test to assert on, so any spec written then would have tested the store directly (not L3) or passed vacuously. It moved, with the read path, into the follow-up carved out in `proposal.md`.
  - **Resolved:** `serve-retained-remote-transcripts` shipped the read route plus remote-origin hydration, and with them this scenario. No part of the deferral still stands.
