...
- Serves the built web client as static files (production) or proxies to Vite dev server (dev mode). Cache: `/assets/*` (Vite content-hashed output) → `Cache-Control: public, max-age=31536000, immutable`; `assets` path-segment check via `path.sep` in `packages/server/src/server.ts` `setHeaders`; precompressed `.gz` siblings inherit header. `.html` + SPA fallback → `no-store`. Other root static files (`sw.js`, `manifest.json`) → @fastify/static default. See change: trim-cold-start-transfer-and-config-fanout (①).
...
- **`endedTotals`.** Ended count per group key regardless of window, every group with ≥1 ended session — CAPPED at `SNAPSHOT_ENDED_TOTALS_GROUPS = 25` groups ranked by max `endedSortKey` (most recent ended activity). Dropped SESSIONS (not groups) counted in `SessionsSnapshotMessage.endedTotalsOverflow` — omitted when 0. Capped-out groups stay fully pageable via `sessions_page` — never orphaned. See change: fix-archive-feedback-and-sidebar-perf.
- **Key folding.** Every `endedTotals` key, archive-index `groupPath` row, and `sessions_page.cwd` folds through the `pathKey` space (`groupKeyOf` = `pathKey(resolveOrderKey(...), platform)`) — `/a/b` and `/a/b/` are ONE key; no count drift on cosmetic path variants. See change: fix-archive-feedback-and-sidebar-perf.
...
- **Client merge semantics.** Snapshot REPLACES `sessions` + `sessionOrderMap` atomically (paged rows discarded on reconnect — documented trade-off); `sessions_page_result` merges — sessions overwrite by id, order appends after current with held ids deduped. Stub group renders for any group key with `endedTotals > 0` and no held session (header + ended expander only); expander / "more" click pages while `heldEnded < endedTotal`; one in-flight `sessions_page` per cwd (released on reply, 15 s timeout, socket open). `STUB_GROUP_BUDGET = 8` (`packages/client/src/components/session/SessionList.tsx`): zero-session stub groups beyond 8 in the DEFAULT unfiltered view collapse into ONE `stub-budget-overflow` summary row ("+N more folders") — click expands. Session-bearing groups (ended included) never consume the budget; pinned stub groups always render; any active narrowing filter renders all matching stubs uncapped. See change: fix-archive-feedback-and-sidebar-perf.
...
**User image attachments.** `prepareEventForIngest()` replaces image payloads with pending placeholders when fitting runs; `attachment_fitted` patches them later. Without a fit worker, under-ceiling image blocks may remain. `event-reducer.ts` derives `imageCount` and retained images from user message blocks. `ChatView` renders the localized attachment-count notice and fitted thumbnails/lightbox. User prompt retry preserves image data.
...
**Client lifetime bounds + subscription release** (change: fix-archive-feedback-and-sidebar-perf). `replay-persist` (`packages/client/src/lib/replay/replay-persist.ts`) trims each session's in-memory raw-event buffer to the server's replay window after every successful flush (`retainLimit = max(DEFAULT_MEMORY_LIMITS.maxReplayEvents, MIN_REPLAY_WINDOW)`); persisted `maxSeq` stays the FULL buffer's max; events appended during an in-flight IndexedDB write survive the trim. Client releases a session's subscription on deselect/switch: sends `unsubscribe { sessionId }` + drops the session from the subscribed-guard set (so a later re-selection re-subscribes). Held (release AND ref advance frozen) while a plugin overlay route is matched. NO unload beacon — server tears down per-connection subscription state on socket close. Pure decision: `resolveSubscriptionTransition(prev, next, overlayMatched)` in `packages/client/src/lib/session/subscription-transition.ts` (capability `session-subscription-lifecycle`). Display-name basename fallback gains suffix `" · " + id.slice(0,8)` (`session-display-name.ts`) so unnamed sessions in one folder stop rendering identical labels. `useInitStatus` gains a module-level resolved-cwd cache + in-flight dedup (`packages/client/src/lib/git/init-status-cache.ts`) — remounts of a resolved folder probe ZERO times; per-paint probe count additionally bounded by the stub budget (≤8 default view).
...
#### Client replay publication and render budget

`useMessageHandler` queues `event_replay` batches per session and publishes at most once per animation frame (`setTimeout(0)` while the document is hidden). Live events flush before replay or reset boundaries; replay reset preserves `pendingPrompt`. Per-session queues prevent one large transcript from blocking another session or every composer keystroke. Loading ends only after the terminal replay marker, including empty-session markers.

Bridge `message_update` handling uses a session-scoped `MessageUpdateCoalescer` (`packages/extension/src/message-update-coalescer.ts`). Text snapshots use a fixed 50 ms window, last snapshot wins, and the window never debounces indefinitely. Thinking, tool, and unknown update types flush pending text before forwarding. Every non-update boundary flushes before `message_end`, `message_start`, or pass-through events. Message generation plus `role:timestamp` key drops stale updates after message end, session switch, reconnect, or shutdown.

User image rows derive `imageCount` and retained images from user message blocks; retained or fitted images render as thumbnails/lightbox content. Completed-message replay compaction remains replay-only and preserves sequence cursors, streaming tails, thinking updates, and reducer output. The newer `compactEventsForReplay` contract supersedes any older one-pass implementation details. Both replay limits remain active: initial subscribe sends `historyWindow:{messages:200}` and terminal replay carries `HistoryWindowMetadata`; `App` stores it and `ChatView` offers `Load full history` via unwindowed `lastSeq:0` subscribe. Separately, `maxReplayEvents`/`replayWindowMode` creates event-count gaps handled by `history_backfill`.

#### Compact workspace sidebar

`useSidebarState` stores the compact flag in browser-local state. `SessionList` keeps folder actions and session cards while reducing folder padding, auxiliary sections, and gaps. Compact mode preserves the final folder basename with `shrink-0 whitespace-nowrap`; parent path may truncate. Settings exposes the toggle in both settings route and overlay. Disabling compact mode remounts hidden folder surfaces.
...
- Ack (change: fix-archive-feedback-and-sidebar-perf): `handleArchiveSession` acks EVERY outcome via `archive_result { sessionId, ok, pending?, error?, code? }` — codes `not_found` / `reject_live` / `reject_running` / `unavailable` / `failed`; REST route unchanged. Client toasts `err.archive.*` (en + hu) for `ok:false`; `ok`/`pending` quiet; no optimistic removal. `archive_result` + `session_archived` are state-class frames (see Frame delivery policy). Live-flag self-heal: cold-start normalization clears the in-memory `live` flag on rows it normalizes (recovery revoke / grace-expiry too), so a crash-survived flag cannot reject archive forever; pending end-then-archive intent consumed at the `ended` TRANSITION (`onEnded`, `consumePendingArchiveIntent`) — covers `update({status:"ended"})` paths (force-kill, session-moved), not just `onUnregister`.
